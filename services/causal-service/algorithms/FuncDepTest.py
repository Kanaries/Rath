...
class FuncDepTestParams(common.OptionalParams, title="FuncDepTest Algorithm"):
    ...
    alpha: Optional[float] = Field(
        default=math.log10(0.0005), title="log10(显著性阈值)", # "Alpha",
...
class FuncDepTest(common.AlgoInterface):
    ...
    def calc(self, params: Optional[ParamType] = ParamType(), focusedFields: List[str] = [], bgKnowledgesPag: Optional[List[common.BgKnowledgePag]] = []):
        ...
        print(coeff_p)
        linear_threshold = 1e-12
        threshold = 10 ** params.o_alpha
        for i in range(d):
            for j in range(i):
                if linear_threshold < coeff_p[i, j] < 10 ** params.alpha:
                    ...
                    if a * threshold < b:
                        ...
                        res[i, j], res[j, i] = -1, -1
                elif coeff_p[i, j] <= linear_threshold: # linear
                    res[i, j], res[j, i] = 1, 1
