...
// @ts-ignore
const programWorker = new Worker(new URL('./program.worker', import.meta.url)) as Worker;
...
export const createProgram = (
    ...
    for (const f of fields) {
        ...
        const col = data.map((row) => (f.mode === 'text' ? String : f.mode === 'bool' ? (d: any) => (d ? 1 : 0) : Number)(row[f.fid])) as
            | number[]
            | string[]
            | (0 | 1)[];
        columns.push({
...
export const resolveFields = (tokens: Static<FieldToken[]>): IRawField[] => {
    return tokens.map<IRawField>((token) => ({
        ...
        semanticType:
            token.extInfo?.extOpt === 'dateTimeExpand'
                ? token.extInfo.extInfo === 'utime'
                    ? 'temporal'
                    : token.extInfo.extInfo === '$y'
                    ? 'quantitative'
                    : 'ordinal'
                : (
                      {
