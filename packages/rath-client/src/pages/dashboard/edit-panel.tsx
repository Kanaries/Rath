...
const EncodingField: FC<{
    id: typeof EncodingKeys[number]; data: IVegaSubset['encoding']; fieldsMeta: IFieldMeta[]; onChange: (value: IVegaChannel | null) => void;
    ...
    const handleSelectField = useCallback((_: unknown, item?: IDropdownOption) => {
        const { key } = item ?? {};
        if (!key) {
            return;
        }
            ...
            <div>
                ...
                {field && (
                    <Dropdown
                        ...
                        onChange={(_, item) => value && item && (AggregationTypes as readonly string[]).includes(item.key as string) && onChange({
                            ...value,
                ...
                {field && (
                    <Toggle
                        ...
                        inlineLabel
                        checked={bin === true}
                        onChange={(_, checked) => value && onChange({
                            ...value,
                            bin: checked,
                        })}
                    />
