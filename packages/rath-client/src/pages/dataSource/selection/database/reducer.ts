...
const useDatabaseReducer = (setLoadingAnimation: (show: boolean) => void) => {
    ...
    const setProgress: typeof _setProgress = useCallback(p => {
    ...
    const [
        connectorReady$, databaseType$, connectUri$, sourceId$, database$, schema$, table$, queryString$, preview$
    ] = useMemo(() => [
    ...
    useEffect(() => {
        const subscriptions = [
            connectorReady$.subscribe(cReady => {
                _setProgress(p => produce(p, draft => {
            ...
            schema$.pipe(
                ...
                if (!hasTableList || !tableEnumerable || sourceId.status !== 'resolved') {
                    ...
                    (hasDatabase ? typeof dbName === 'string' : true)
                    && (requiredSchema ? typeof schemaName === 'string' : true)
                ) {
                    table$.next(Promise.resolve({
                        status: 'pending',
                        options: [],
    ...
    }, [connectorReady$, databaseType$, connectUri$, sourceId$, database$, schema$, table$, preview$, queryString$, setLoadingAnimation, setProgress]);
    ...
    const actions = useMemo(() => ({
        ...
        setSchema: (schema: DatabaseOptions['schema']['options'][0]) => {
            schema$.next(Promise.resolve(produce(ref.current.schema, draft => {
                draft.value = schema;
            })));
        },
