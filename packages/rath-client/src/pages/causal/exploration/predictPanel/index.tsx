...
>(function PredictPanel(_, ref) {
    ...
    return (
        <Container>
            <div className="ml-[0.6em] inline-flex w-max grow-0 shrink-0">
                <Button
                    type="button"
                ...
                >
                    {running ? <Spinner style={{ transform: 'scale(0.75)' }} /> : <RathIcon name="Play" />}
                    <span>{`${ModeOptions.find((m) => m.key === mode)?.text}${intl.get('causal.analyze.prediction')}`}</span>
                </Button>
                <DropdownMenu>
