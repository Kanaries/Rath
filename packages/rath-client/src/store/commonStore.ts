...
import { getAvatarURL, getMainServiceUrl, AVATAR_IMG_LIST, IAVATAR_TYPES } from '../utils/user';
...
export class CommonStore {
    ...
    public async liteAuth(certMethod: 'email' | 'phone') {
        const url = getMainServiceUrl('/api/liteAuth');
    ...
    public async commitLogout() {
        try {
            const url = getMainServiceUrl('/api/logout');
            const res = await fetch(url, {
                method: 'GET',
            });
            if (res) {
    ...
    public async updateAuthStatus() {
        try {
            const url = getMainServiceUrl('/api/loginStatus');
    ...
    public async getPersonalInfo() {
        const url = getMainServiceUrl('/api/ce/personal');
        try {
    ...
    public async customAvatar(value: { file?: File; isDefaultAvatar?: boolean }) {
        try {
            ...
            const url = getMainServiceUrl('/api/ce/avatar');
            const res = await fetch(url, {
                method: 'POST',
                credentials: 'include',
