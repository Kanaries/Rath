...
function specification (dataSource, dimensions, measures) {
  const { dimScores, newDimensions, aggData } = fieldsAnalysis(dataSource, dimensions, measures);
  // todo
  // use newDimensions to replace viewDimensions
  let viewDimensions = dimScores.map(dim => dim[0]);
  // deleta measures in dimScores
  ...
  if (schema.position.length === 2) {
    ...
    // }
    console.log('schema', schema, viewDimensions, measures)
